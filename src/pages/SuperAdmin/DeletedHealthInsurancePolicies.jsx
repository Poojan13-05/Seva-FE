...
const DeletedHealthInsurancePolicies = () => {
  ...
  const fetchDeletedPolicies = async (page = 1) => {
    try {
      setLoading(true);
      const response = await superAdminService.getDeletedHealthInsurancePolicies({
        page,
      ...
      if (response.success) {
        ...
        toast.error('Failed to fetch deleted health insurance policies');
      }
    } catch (error) {
  ...
  const fetchDeletedPolicyStats = async () => {
    try {
      const response = await superAdminService.getDeletedHealthInsuranceStats();
      if (response.success) {
  ...
  const confirmPermanentDelete = async () => {
    ...
    try {
      ...
      const response = await superAdminService.permanentlyDeleteHealthInsurancePolicy(policy._id);
      ...
      if (response.success) {
        ...
        toast.error('Failed to permanently delete health insurance policy');
      }
    } catch (error) {
  ...
  const confirmRecover = async () => {
    ...
    try {
      ...
      const response = await superAdminService.recoverHealthInsurancePolicy(policy._id);

      if (response.success) {
        toast.success('Health insurance policy recovered successfully');
        fetchDeletedPolicies(pagination.currentPage);
        fetchDeletedPolicyStats();
        setConfirmRecoverDialog({ open: false, policy: null });
      } else {
        toast.error('Failed to recover health insurance policy');
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">Deleted Health Insurance Policies</h1>
          <p className="text-gray-400 mt-2">
            Manage permanently deleted health insurance policies and cleanup data
          </p>
        </div>
        <Button
            ...
            <div className="flex flex-col sm:flex-row gap-4 flex-1">
              ...
              <Select value={filters.policyType} onValueChange={(value) => handleFilterChange('policyType', value)}>
                ...
                <SelectContent className="bg-white/10 backdrop-blur-md border-white/20">
                  ...
                  <SelectItem value="Portability" className="text-gray-300 hover:text-white">
                    Portability
                  </SelectItem>
                </SelectContent>
              </Select>
      ...
      <Card className="bg-white/10 backdrop-blur-md border-white/20">
        <CardHeader>
          <CardTitle className="text-white">Deleted Health Insurance Policies</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-400">No health insurance policies have been deleted yet.</p>
            </div>
          ) : (
            <>
              <Table>
                <TableHeader>
                  <TableRow className="border-white/20 hover:bg-white/5">
                    ...
                    <TableHead className="text-gray-300">Family Members</TableHead>
              ...
              {confirmDeleteDialog.policy && (
                <>
                  ...
                  <ul className="list-disc list-inside text-sm space-y-1 ml-4">
                    ...
                    <li>Family details ({confirmDeleteDialog.policy.familyDetails?.length || 0} members)</li>
            ...
            <DialogDescription className="space-y-2 text-gray-300">
              {confirmRecoverDialog.policy && (
                <>
                  ...
                  <ul className="list-disc list-inside text-sm space-y-1 ml-4">
                    <li>Policy details and client information</li>
                    <li>Family details ({confirmRecoverDialog.policy.familyDetails?.length || 0} members)</li>
...
export default DeletedHealthInsurancePolicies;
