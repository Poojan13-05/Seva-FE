...
const DeletedVehicleInsurancePolicies = () => {
  ...
  const fetchDeletedPolicies = async (page = 1) => {
    try {
      setLoading(true);
      const response = await superAdminService.getDeletedVehicleInsurancePolicies({
        page,
      ...
      if (response.success) {
        ...
        toast.error('Failed to fetch deleted vehicle insurance policies');
      }
    } catch (error) {
  ...
  const fetchDeletedPolicyStats = async () => {
    try {
      const response = await superAdminService.getDeletedVehicleInsuranceStats();
      if (response.success) {
  ...
  const confirmPermanentDelete = async () => {
    ...
    try {
      ...
      const response = await superAdminService.permanentlyDeleteVehicleInsurancePolicy(policy._id);

      if (response.success) {
        toast.success('Vehicle insurance policy permanently deleted successfully');
        fetchDeletedPolicies(pagination.currentPage);
        fetchDeletedPolicyStats();
        setConfirmDeleteDialog({ open: false, policy: null });
      } else {
        toast.error('Failed to permanently delete vehicle insurance policy');
      }
    } catch (error) {
  ...
  const confirmRecover = async () => {
    ...
    try {
      ...
      const response = await superAdminService.recoverVehicleInsurancePolicy(policy._id);

      if (response.success) {
        toast.success('Vehicle insurance policy recovered successfully');
        fetchDeletedPolicies(pagination.currentPage);
        fetchDeletedPolicyStats();
        setConfirmRecoverDialog({ open: false, policy: null });
      } else {
        toast.error('Failed to recover vehicle insurance policy');
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">Deleted Vehicle Insurance Policies</h1>
          <p className="text-gray-400 mt-2">
            Manage permanently deleted vehicle insurance policies and cleanup data
          </p>
        </div>
        <Button
            ...
            <div className="flex flex-col sm:flex-row gap-4 flex-1">
              ...
              <Select value={filters.policyType} onValueChange={(value) => handleFilterChange('policyType', value)}>
                ...
                <SelectContent className="bg-white/10 backdrop-blur-md border-white/20">
                  ...
                  <SelectItem value="Rollover" className="text-gray-300 hover:text-white">
                    Rollover
                  </SelectItem>
                </SelectContent>
              </Select>
      ...
      <Card className="bg-white/10 backdrop-blur-md border-white/20">
        <CardHeader>
          <CardTitle className="text-white">Deleted Vehicle Insurance Policies</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-400">No vehicle insurance policies have been deleted yet.</p>
            </div>
          ) : (
            <>
              <Table>
                <TableHeader>
                  <TableRow className="border-white/20 hover:bg-white/5">
                    ...
                    <TableHead className="text-gray-300">Registration Number</TableHead>
                  ...
                  {policies.map((policy) => (
                    <TableRow key={policy._id} className="border-white/20 hover:bg-white/5">
                      <TableCell className="text-white">
                        <div className="space-y-1">
                          ...
                          <div className="text-xs text-gray-400">
                            {policy.insuranceDetails?.make || ''} {policy.insuranceDetails?.model || ''}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="text-white">
                      ...
                      <TableCell className="text-gray-300">
                        ...
                        <div className="text-xs text-gray-400">
                          {policy.insuranceDetails?.agencyCode || 'N/A'}
                        </div>
                      </TableCell>
                      <TableCell>
                        ...
                        <div className="text-xs text-gray-400 mt-1">
                          {policy.insuranceDetails?.vehicleType || 'N/A'}
                        </div>
                      </TableCell>
                      <TableCell className="text-white">
                        <div className="font-medium text-sm">
                          {policy.insuranceDetails?.registrationNumber || 'N/A'}
                        </div>
                        <div className="text-xs text-gray-400">
                          {policy.insuranceDetails?.engineNumber
                            ? `Engine: ${policy.insuranceDetails.engineNumber.substring(0, 10)}...`
                            : 'N/A'}
                        </div>
                      </TableCell>
                      <TableCell className="text-gray-300">
...
export default DeletedVehicleInsurancePolicies;
