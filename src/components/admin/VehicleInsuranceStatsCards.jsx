// src/components/admin/VehicleInsuranceStatsCards.jsx
...
import { useVehicleInsuranceStats } from '@/hooks/useVehicleInsurance';

const VehicleInsuranceStatsCards = () => {
  const { data, isLoading, error } = useVehicleInsuranceStats();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6">
        <Card className="bg-white/10 backdrop-blur-md border-white/20 col-span-full">
          <CardContent className="flex items-center justify-center py-8">
            <p className="text-red-400">Failed to load vehicle insurance statistics</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  const statsCards = [
    {
      ...
      description: "Active vehicle insurance policies"
    },
    {
      title: "New Policies",
    ...
    {
      title: "Rollover Policies",
      value: stats?.rolloverPolicies || 0,
      icon: RotateCw,
      iconColor: "text-purple-400",
      bgColor: "bg-purple-500/20",
      description: "Policy rollovers",
      badge: stats?.rolloverPolicyPercentage ? `${Math.round(stats.rolloverPolicyPercentage)}%` : null
    },
    {
      title: "Total Premium",
...
export default VehicleInsuranceStatsCards;
